...
public class CheckoutCreateView {
    private BorderPane root = new BorderPane();
    private InventoryController controller;
    private AdminView view;
    private Product product;

	public CheckoutCreateView(AdminView view, Product product) {
		this.view = view;
	...
	private void init() {
        ...
        TextField totalPriceField = ViewUtil.createTextFieldWithText(product.getUnitPrice().toString());
                ...
                try {
                    ...
                    if (validator.isValid()) {
                        if (quantity > 0 && quantity <= inventory.getAvailableItems()) {
                            controller.checkoutInventory(item);
